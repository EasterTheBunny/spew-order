package main

import (
	"log"
	"strings"

	"github.com/easterthebunny/spew-order/internal/funding"
	"github.com/easterthebunny/spew-order/pkg/types"
)

func main() {

	src := strings.NewReader(`-----BEGIN PUBLIC KEY-----

-----END PUBLIC KEY-----`)

	config := funding.SourceConfig{
		CallbackAudit: log.Writer(),
		PublicKey:     src,
		APIKey:        "<apikey>",
		APISecret:     "<apisecret>",
	}
	s := funding.NewCoinbaseSource(config)

	a, err := s.CreateAddress(types.SymbolEthereum)
	if err != nil {
		log.Println(err.Error())
	} else {
		log.Printf("id: %s; hash: %s", a.ID, a.Hash)
	}
}
